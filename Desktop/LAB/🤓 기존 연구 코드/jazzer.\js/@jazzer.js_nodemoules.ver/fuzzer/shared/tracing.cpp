...
// libfuzzer의 FuzzerTracePC.cpp에 있는 내용 호출
extern "C" {
void __sanitizer_weak_hook_strcmp(void *called_pc, const char *s1, // pc는 프로그램 카운터
...
void TraceStringContainment(const Napi::CallbackInfo &info) {
  ...
  // operation. 이 값은 실제로는 서브스트링이 발견된 위치를 반환하지만, 후크에서는 이를 무시하고 서브스트링 검색이 일어났다는 사실만 기록
